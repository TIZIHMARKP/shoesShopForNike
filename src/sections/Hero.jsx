    ...
    <section className="w-full flex xl:flex-row flex-col justify-center min-h-screen gap-10 max-container">
      ...
      <div className="relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28">
        ...
        <div className="flex justify-start items-start flex-wrap w-full mt-20 gap-16">
          {statistics.map((stat) => ( // using key of i is not the best practice because we have label which is unique in our object, meaning we don't need to have index(i), so we can use stat.label
            <div key={stat.label}>  
          ...
          {shoes.map((shoe) => (
            <div key={shoe}>
              <ShoeCard 
                ...
                bigShoeImg={bigShoeImg} // to know which one is correctly selected
              />
            </div>
          ))}
        </div>
